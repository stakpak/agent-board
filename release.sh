...
update_cargo_version() {
    ...
    if cargo update --package agent-board; then
...
# Main script logic
main() {
    print_info "Starting release process for agent-board..."
    ...
    if [[ "$is_beta" == true ]]; then
        ...
        print_info "  curl -L https://github.com/stakpak/agent-board/releases/download/v${new_version}/agent-board-darwin-aarch64.tar.gz | tar xz"
        print_info "  sudo mv agent-board /usr/local/bin/"
    else
        print_success "Release $new_version completed successfully! 🎉"
        print_info "Homebrew formula will be updated automatically."
        print_info "Install via: brew install stakpak/stakpak/agent-board"
    fi
